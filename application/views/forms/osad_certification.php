<?php
 ...
 //    for($x=0;$x<count($result);$x++)
	// {
		...
		$this->pdf->Ln();
		$this->pdf->SetFont('Times','',14);
		$this->pdf->MultiCell(0,5, 'MRS. ESTRELLA SOBRECREY',0,'R');
		$this->pdf->SetFont('Times','',12);
		$this->pdf->MultiCell(0,5, 'Head of Discipline(Female)',0,'R');
		$this->pdf->MultiCell(0,5, 'UIC - Main Campus',0,'R');
		$this->pdf->Ln();
		$this->pdf->Ln();
		$this->pdf->Ln();
		$this->pdf->SetFont('Times','',14);
		$this->pdf->MultiCell(0,5, 'MR. EDGARDO GRANDEZA',0,'R');
		$this->pdf->SetFont('Times','',12);
		$this->pdf->MultiCell(0,5, 'Head of Discipline(Male)',0,'R');
		$this->pdf->MultiCell(0,5, 'UIC - Annex Campus',0,'R');
		$this->pdf->Ln();
		$this->pdf->Ln();
		$this->pdf->Ln();
		$this->pdf->SetFont('Times','',14);
		$this->pdf->MultiCell(0,5, 'MRS. ANAFLOR SACOPAYO',0,'R');
		$this->pdf->SetFont('Times','',12);
		$this->pdf->MultiCell(0,5, 'Head of Student Affairs',0,'R');
		$this->pdf->MultiCell(0,5, 'UIC - Main and Annex Campus',0,'R');
        $this->pdf->Output();
?>